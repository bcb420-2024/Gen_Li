...
The normalized data has HUGO gene symbols as its row names, sample IDs/conditions as its column names, each cell of the matrix contains the normalized expression value of each gene in each sample (shown in the following Figure).



![Figure.0 The format for the normalized expression data from assignment 1.](data_intro.png)
...
The gene sets that I use is the human pathway definition with GO biological processes, but without IEA (inferred from electronic annotation), which is downloaded directly from the [Dr.Bader's Lab website](http://download.baderlab.org/EM_Genesets/current_release/Human/symbol/). The version for the gene sets file is the latest version, which is April_01_2024 when I finish this assignment 3.
...
![Figure.11 The ready-for-publication ENrichment Map with legends.](Ready_publish_EM.png) Figure.11 The ready-for-publication ENrichment Map with legends.
<br>
