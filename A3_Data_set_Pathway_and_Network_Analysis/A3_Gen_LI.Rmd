...
```
After downloaded the pathway definition file, the GSEA is going to be performed locally on my computer, since I am using an M1 chip MacBook, amd I am using the arm64 image for docker. I always get an error when running GSEA from R, which may be caused by the compatibility issue. Therefore, I decided to run it through the desktop interface since it is could be run properly. Also the app for Mac always be terminated for no reason on M1 chip macbook, so I use my intel chip computer to run it. 
## Data Loading
...
![Figure.1 The interface for the data loading for GSEA.](load_data.jpg)

## Parameter Adjustment

Select "GseaPreRanked" button under the "Tools" menu to adjust parameters. There are a few parameters need specifying (Figure.2):
...
![Figure.2 The parameters used for the GSEA.](Parameters.jpg)
...
![Figure.3 The index file for the analysis result of GSEA.](analysis_result.jpg)


There are two "Enrichment in Phenotype" sections. The first section shows the results for gene sets that have a positive enrichment score (gene sets that show enrichment at the top of the ranked list) and the second section shows results for gene sets that have a negative enrichment score (gene sets that show enrichment at the bottom of the ranked list) (Figure.3).

It is easy to see that we have 7683 gene sets included in the analysis, 5060 of them are enriched with up-regulated genes, 2623 of them are enriched with down-regulated genes (Figure.3). 

Among those 5060 gene sets with positive enrichment scores, 548 gene sets are significantly enriched with a false discovery rate (FDR) less than 25%, 369 gene sets have p-value less than 0.01, 849 gene sets have p-value less than 0.05 (Figure.3). 

And among those 2623 gene sets, 1107 gene sets are significantly enriched with a false discovery rate (FDR) less than 25% as well, 596 gene sets have p-value less than 0.01, 931 gene sets have p-value less than 0.05 (Figure.3).  
...
![Figure.4 The top 9 enrichment plots with the highest absolute normalized enrichment scores for Up-regulated genes.](pos_snapshot.jpg) 



**Enrichment Plots for Down-regulated genes:**

![Figure.5 The top 9 enrichment plots with the highest absolute normalized enrichment scores for Down-regulated genes.](neg_snapshot.jpg)
...
The gene sets that I use is the human pathway definition with GO biological processes, but without IEA (inferred from electronic annotation), which is downloaded directly from the Dr.Bader's Lab website. The version for the gene sets file is the latest version, whose last-modified data is March_19_2024 when I finish this assignment 3. 
...
![Figure.6 Files and parameters used for crating the Enrichment Map in Cytoscape with App EnrichmentMap.](EM_parameters.png)


This is the Enrichment Map generated without manual layout:


![Figure.7 The raw output of the Enrichment Map (before manual layout).](EM_before_manual_layout.png)
...
![Figure.8 Parameters used for the annotation of the Enrichment Map.](Annotate_parameters.png)


Below is the initial result of annotation of the enrichment map:


![Figure.9 The result for the Enrichment Map annotaion without adjusting.](EM_with_annotation.png)

After optimizing and adding legend, this is the current ready to publish version of the enrichment map:

![Figure.10 The ready-for-publication ENrichment Map with legends.](Ready_publish_EM.png)
...
There are 196 nodes and 3246 edges in the resulting map. I set the FDR q-value as 0.0001 to create this map, and the threshold for edge similarity is 0.375. The screenshot of your network prior to manual layout can be found above in Figure.7.
...
The network is collapsed to a theme network, where each cluster are collapsed into a single node (Figure.11). The major theme present in this analysis is the metabolism (synthesis and degradation) and process related to cholesterol metabolism in hepatocytes (Figure.10). These themes fit well with the model, since the model described in the original [paper](https://pubmed.ncbi.nlm.nih.gov/35534729/)[@Cheng2022] shows the relationship between the experiment conditions with the synthesis of fatty acid and cholesterol. No novel pathways or themes are found.
![Figure.11 The collapsed Enrichment Map, which can be found that pathways that are up-regulated are highly-related to process related to cholesterol.](collapsed_EM.png)
...
![Figure.12 The result for the post-analysis, with signature gene sets of Doconexent, Fish Oil and Omega-3 fatty acids.](post_analysis.png)
